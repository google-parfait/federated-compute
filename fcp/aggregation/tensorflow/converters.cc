...
StatusOr<TensorSpec> ToAggTensorSpec(
  ...
  FCP_ASSIGN_OR_RETURN(DataType dtype, ToAggDataType(spec.dtype()));
...
StatusOr<Tensor> ToAggTensor(std::unique_ptr<tf::Tensor> tensor) {
  FCP_ASSIGN_OR_RETURN(DataType dtype, ToAggDataType(tensor->dtype()));
...
absl::StatusOr<tf::TensorShape> ToTfShape(
    const ::fcp::aggregation::TensorShape& shape) {
...
absl::StatusOr<tf::Tensor> ToTfTensor(Tensor tensor) {
  ...
  FCP_ASSIGN_OR_RETURN(tf::DataType dtype, ToTfDataType(tensor.dtype()));
  FCP_ASSIGN_OR_RETURN(tf::TensorShape shape, ToTfShape(tensor.shape()));

  tsl::core::RefCountPtr<tf::TensorBuffer> tensor_buffer;
  if (tensor.dtype() == DT_STRING) {
    // Views the string_views in the FCP Tensor to an array of TensorFlow
