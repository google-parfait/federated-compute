...
TEST_F(OpStatsExampleStoreTest, TestInvalidCollectionUrl) {
  ...
  absl::StatusOr<std::unique_ptr<ExampleIterator>> status_or =
      OpStatsExampleStore::CreateExampleIterator(selector, mock_db_,
                                                 mock_log_manager_);
...
TEST_F(OpStatsExampleStoreTest, TestMalformedCriteria) {
  ExampleSelector selector;
  selector.set_collection_uri(OpStatsExampleStore::kOpStatsCollectionUri);
...
TEST_F(OpStatsExampleStoreTest, Success) {
  ...
  actual_session_names.insert(
      ExtractSingleString(example_last, OpStatsExampleStore::kSessionName));
  actual_session_names.insert(
      ExtractSingleString(example_first, OpStatsExampleStore::kSessionName));
  ...
  actual_population_names.insert(
      ExtractSingleString(example_last, OpStatsExampleStore::kPopulationName));
  actual_population_names.insert(
      ExtractSingleString(example_first, OpStatsExampleStore::kPopulationName));
...
TEST_F(OpStatsExampleStoreTest, DataIsFilteredBySelectionCriteria) {
  ...
  *criteria.mutable_end_time() = TimeUtil::MillisecondsToTimestamp(2000L);
  ...
  auto event_type_list =
      ExtractRepeatedInt64(example, OpStatsExampleStore::kEventsEventType);
  ...
  auto event_time_ms_list = ExtractRepeatedInt64(
      example, OpStatsExampleStore::kEventsTimestampMillis);
...
TEST_F(OpStatsExampleStoreTest, FullSerialization) {
  ...
  *opstats_sequence.add_opstats() = std::move(stats);
  ...
  // Singular fields
  ASSERT_EQ(ExtractSingleString(example, OpStatsExampleStore::kSessionName),
            session);
  ASSERT_EQ(ExtractSingleString(example, OpStatsExampleStore::kPopulationName),
            population);
  ASSERT_EQ(ExtractSingleString(example, OpStatsExampleStore::kTaskName),
            task_name);
  ASSERT_EQ(ExtractSingleString(example, OpStatsExampleStore::kErrorMessage),
            error);
  ASSERT_EQ(ExtractSingleInt64(example, OpStatsExampleStore::kBytesDownloaded),
            bytes_downloaded);
  ASSERT_EQ(ExtractSingleInt64(example, OpStatsExampleStore::kBytesUploaded),
            bytes_uploaded);
  ASSERT_EQ(ExtractSingleInt64(
                example, OpStatsExampleStore::kChunkingLayerBytesDownloaded),
            chunking_layer_bytes_downloaded);
  ASSERT_EQ(ExtractSingleInt64(
                example, OpStatsExampleStore::kChunkingLayerBytesUploaded),
  ...
  auto event_times = ExtractRepeatedInt64(
      example, OpStatsExampleStore::kEventsTimestampMillis);
  ...
  // Dataset stats
  auto dataset_urls =
      ExtractRepeatedString(example, OpStatsExampleStore::kDatasetStatsUri);
  ...
  auto example_counts = ExtractRepeatedInt64(
      example, OpStatsExampleStore::kDatasetStatsNumExamplesRead);
  ASSERT_EQ(example_counts.at(index_a), num_examples_a);
  ASSERT_EQ(example_counts.at(1 - index_a), num_examples_b);
  auto example_bytes = ExtractRepeatedInt64(
      example, OpStatsExampleStore::kDatasetStatsNumBytesRead);
  ASSERT_EQ(example_bytes.at(index_a), example_bytes_a);
  ASSERT_EQ(example_bytes.at(1 - index_a), example_bytes_b);

  // RetryWindow
  ASSERT_EQ(ExtractSingleInt64(example,
                               OpStatsExampleStore::kRetryWindowDelayMinMillis),
            min_delay_ms);
  ASSERT_EQ(ExtractSingleInt64(example,
                               OpStatsExampleStore::kRetryWindowDelayMaxMillis),
            max_delay_ms);
}
