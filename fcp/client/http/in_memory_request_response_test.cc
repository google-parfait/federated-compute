...
TEST(InMemoryHttpRequestTest, NonHttpsUriFails) {
  ...
  EXPECT_THAT(request.status(), StatusIs(absl::StatusCode::kInvalidArgument));
}
...
TEST(InMemoryHttpRequestCallbackTest, ResponseFailsBeforeHeaders) {
  ...
  EXPECT_THAT(actual_response, StatusIs(absl::StatusCode::kUnimplemented));
...
TEST(InMemoryHttpRequestCallbackTest,
  ...
  absl::StatusOr<InMemoryHttpResponse> actual_response = callback.Response();
  EXPECT_THAT(actual_response, StatusIs(absl::StatusCode::kInvalidArgument));
...
TEST(InMemoryHttpRequestCallbackTest,
  ...
  EXPECT_THAT(response_body_result, StatusIs(absl::StatusCode::kOutOfRange));
...
TEST(InMemoryHttpRequestCallbackTest, ResponseWithContentLengthNegative) {
  ...
  EXPECT_THAT(response_started_result, StatusIs(absl::StatusCode::kOutOfRange));
...
TEST(InMemoryHttpRequestCallbackTest, NotFoundResponse) {
  ...
  absl::StatusOr<InMemoryHttpResponse> actual_response = callback.Response();
  EXPECT_THAT(actual_response, StatusIs(absl::StatusCode::kNotFound));
...
TEST_F(PerformRequestsTest, PerformTwoRequestsWithOneFailedOneSuccess) {
  ...
  EXPECT_THAT(results->at(1), StatusIs(absl::StatusCode::kNotFound));
...
TEST_F(PerformRequestsTest, FetchResourcesInMemoryAllUris) {
  ...
  EXPECT_THAT((*result)[1], StatusIs(absl::StatusCode::kNotFound));
  EXPECT_THAT((*result)[1].status().message(), HasSubstr("404"));
  EXPECT_THAT((*result)[2], StatusIs(absl::StatusCode::kUnavailable));
...
TEST_F(PerformRequestsTest, FetchResourcesInMemorySomeInlineData) {
  ...
  EXPECT_THAT((*result)[0], StatusIs(absl::StatusCode::kUnavailable));
...
TEST_F(PerformRequestsTest,
  ...
  EXPECT_THAT((*result)[0], StatusIs(absl::StatusCode::kInternal));
  EXPECT_THAT((*result)[1], StatusIs(absl::StatusCode::kInternal));
