...
TEST_F(FlRunnerExampleQueryTest,
       RemoveTfV1CheckpointSupportEnabledTfV1CheckpointAggregationFails) {
  EXPECT_CALL(mock_flags_, enable_remove_tfv1_checkpoint_support())
      .WillRepeatedly(Return(true));
  AggregationConfig aggregation_config;
  aggregation_config.mutable_tf_v1_checkpoint_aggregation();
  ...
  (*aggregations)["string_tensor"] = aggregation_config;
  (*aggregations)["int_tensor"] = aggregation_config;
  ExpectComputationFailureWithInvalidArgument();
}

TEST_F(FlRunnerExampleQueryTest,
       RemoveTfV1CheckpointSupportDisabledTfV1CheckpointAggregationSucceeds) {
  EXPECT_CALL(mock_flags_, enable_remove_tfv1_checkpoint_support())
      .WillRepeatedly(Return(false));
  AggregationConfig aggregation_config;
  aggregation_config.mutable_tf_v1_checkpoint_aggregation();
