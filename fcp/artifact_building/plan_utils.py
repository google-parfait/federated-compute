...
def convert_graphdef_to_flatbuffer(graph: tf.compat.v1.GraphDef,
  ...
  converter._experimental_guarantee_all_funcs_one_use = guarantee_all_funcs_one_use
...
def generate_and_add_flat_buffer_to_plan(plan: _PlanT,
                                         forgive_tflite_conversion_failure=True
                                        ) -> _PlanT:
  ...
  def convert(graph_def, tensorflow_spec, guarantee_all_funcs_one_use=False):
    stateful_partitioned_call_err = ("'tf.StatefulPartitionedCall' op is" +
                                     ' neither a custom op nor a flex op')
    # Pack the TFLite flatbuffer into a BytesValue proto.
    try:
      return convert_graphdef_to_flatbuffer(graph_def, tensorflow_spec,
                                            guarantee_all_funcs_one_use)
    except Exception as e:  # pylint: disable=broad-except
      # Try to handle conversion errors and run converter again.
      if stateful_partitioned_call_err in str(
          e) and not guarantee_all_funcs_one_use:
