...
std::pair<fcp::confidentialcompute::Key, std::string> GenerateHpkeKeyProtoPair(
  ...
  fcp::confidentialcompute::Key public_key;
  public_key.set_algorithm(
      fcp::confidentialcompute::Key::HPKE_X25519_SHA256_AES128_GCM);
  public_key.set_purpose(fcp::confidentialcompute::Key::ENCRYPT);
  public_key.set_key_id(key_id);
  public_key.set_key_material(std::move(raw_public_key));
  ...
  return {public_key, private_key.value_or(raw_private_key)};
}
